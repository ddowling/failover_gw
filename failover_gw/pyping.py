...
class Ping(object):
    def __init__(self, destination, timeout=1000, packet_size=55, own_id=None, quiet_output=True, udp=False, raw=False, bind=None, source_interface=None, dest_mac=None):
    ...
    def do(self):
        ...
        try: # One could use UDP here, but it's obscure
            if self.raw:
                current_socket = socket.socket(socket.AF_PACKET, socket.SOCK_RAW)
                protocol = 0x0800 # ETH_P_*
                current_socket.bind((self.source_interface, protocol))
    ...
    def send_one_ping(self, current_socket):
        ...
        try:
            if self.raw:
                ...
                ip_hdr += to_ip_components(local_ip)
                ip_hdr += to_ip_components(self.destination)
                ...
                csum_overflow = (csum>>16)
                csum = (csum & 0xffff) + csum_overflow
                csum = csum ^ 0xffff
    ...
    def receive_one_ping(self, current_socket):
        ...
        while True: # Loop while waiting for packet or timeout
            ...
            #packet_hex = " ".join([ "%02x" % ord(c) for c in packet_data])
            #print("recv data={} address={}".format(packet_hex, address))

            # Strip off Ethernet header if a raw packet
            if self.raw:
